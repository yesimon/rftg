                            ...
                            int max_prestige, int max_build)
{
	...
	/* Set inputs for behind leader in built cards */
	for (i = 0; i < 5; i++)
	{
		/* Set input if this many behind */
		eval.input_value[n++] = num_build + i < max_build;
	}
...
static double eval_game(game *g, int who)
{
	...
	int max = 0, max_prestige, max_build = 0, clock;
	...
	for (i = 0; i < 12; i++)
	{
		...
		eval.input_value[n++] = max_build > i;
	}
	...
	n = eval_game_player(g, who, n, max, max_prestige, max_build);
	...
	while (i != who)
	{
		...
		n = eval_game_player(g, i, n, max, max_prestige, max_build);
...
static void dump_game(game *g, game *sim)
{
	...
	{
		printf("Player %d:\n", i);
		printf("%d cards active, %d cards seen, %d cards in hand, %d cards discarded, %d goods\n", count_player_area(sim, i, WHERE_ACTIVE), sim->p[i].total_fake, count_player_area(sim, i, WHERE_HAND) + sim->p[i].fake_hand - sim->p[i].fake_discards, sim->p[i].fake_discards, count_player_area(sim, i, WHERE_GOOD));
...
static int ai_choose_place_opp(game *g, int who, int phase, int special)
{
	...
	/* Sort list of scores */
	qsort(scores, n, sizeof(struct sample_score), cmp_sample_score);

	/* Use second-best score */
	i = n - 2;
                           ...
                           int special, int additional)
{
	...
	/* Check for non-simulated game */
	if (!g->simulation && !additional)
	{
		/* Simulate game */
		simulate_game(&sim, g, who);
		...
		for (i = 0; i < g->num_players; i++)
		{
			...
			which = ai_choose_place_opp(&sim, i, phase, special);
	...
	{
		/* Check for takeovers */
		if (settle_check_takeover(&sim2, who))
		{
			/* Take no-place action */
			settle_action(&sim2, who, -1);
